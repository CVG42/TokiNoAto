...
public class DayNightController : MonoBehaviour
{
    ...
    [SerializeField] private Light2D[] mapLights;
    [SerializeField] private DayNightCycle[] dayCycles;
    ...
    void Start()
    {
        globalLight.color = dayCycles[0].cycleColor;
    }

    void Update()
    {
        currentCycleTime += Time.deltaTime;
        cyclePercentage = currentCycleTime / cycleTime;
        ...
        if(currentCycleTime >= cycleTime)
        {
            ...
            if(nextCycle + 1 > dayCycles.Length - 1)
            {
                nextCycle = 0;
            }
            else
        ...
        ChangeColor(dayCycles[actualCycle].cycleColor, dayCycles[nextCycle].cycleColor);
